...
**Текущий статус:** `Clan War History v1 completed; collection reliability v1 completed; clan snapshot history v1 completed; donations weekly public schema v2 validated; clan_games_v1 Phase 5 scheduler and health implemented`. Production event пока не зарегистрирован, поэтому установленный scheduler должен завершаться безопасным no-op.
...
Текущий публичный сайт использует реальные данные состава, текущей войны и накопленной истории обычных КВ. Коррекция недельных пожертвований на public schema v2 прошла natural production validation: основная метрика показывает прямые игровые counters текущего состава вместо повторного суммирования deltas, а предыдущая неделя использует последнее подтверждённое наблюдение внутри неё. Production reconciliation 26 августа 2026 года сопоставил 42 из 42 строк без расхождений. History schema v2, immutable observations, reconciliation, manual evidence для первых двух старых войн и восстановленные public player metrics завершены. Рейтинг игроков и рекомендации состава не реализованы.

Будущие модули охватывают дальнейшие метрики обычных КВ, ЛВК, рейды и Игры кланов. Недельные пожертвования используют snapshot history и current-roster scope; public schema v2 определяет текущие значения как последние raw counters и previous как последнее наблюдение предыдущей московской недели. Для Игр кланов подтверждён official-derived source: reusable Phase 1 client извлекает lifetime cumulative `Games Champion` из player profile. Phase 2 добавила local-only operator-confirmed event registry, Phase 3 добавила отдельный versioned SQLite store, Phase 4 добавила standalone bounded collector, а Phase 5 добавила deterministic event scheduler, отдельный health store и dedicated hourly task contract. Production event и production DB ещё не созданы; при отсутствии registry scheduler не читает token, не вызывает API и не создаёт DB. Derived points, public projection и frontend не реализованы.
...
Источник состава уже выбран: локальные fail-closed probes официального API. Для `clan_games_v1` source validation и Phase 1-5, включая отдельный event scheduler и health contract, завершены на implementation-уровне. Реальный collection scan требует отдельно зарегистрированного подтверждённого event. Подробности: [docs/clan_games.md](docs/clan_games.md).
