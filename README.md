...
**Текущий статус:** `Clan War History v1 completed; collection reliability v1 completed; clan snapshot history v1 completed; donations weekly v1 completed; clan_games_v1 Phase 2 event registry implemented`. Следующая фаза: separate observation storage.
...
Источник состава уже выбран: локальные fail-closed probes официального API. Для `clan_games_v1` source validation, Phase 1 player client/normalizer и Phase 2 event registry завершены; следующий отдельный этап ограничен observation storage. Подробности: [docs/clan_games.md](docs/clan_games.md).
