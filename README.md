...
**Текущий статус:** `Clan War History v1 completed; collection reliability v1 completed; clan snapshot history v1 completed; donations weekly v1 completed; clan_games_v1 Phase 5 scheduler and health implemented`. Production event пока не зарегистрирован, поэтому установленный scheduler должен завершаться безопасным no-op.
...
Текущий публичный сайт использует реальные данные состава, текущей войны и накопленной истории обычных КВ. Коррекция недельных пожертвований на public schema v2 реализована offline; первый controlled run выявил безопасно обработанный roster-churn edge case, для которого подготовлено offline исправление без retry. Основная метрика показывает прямые игровые counters текущего состава вместо повторного суммирования deltas. History schema v2, immutable observations, reconciliation, manual evidence для первых двух старых войн и восстановленные public player metrics завершены. Рейтинг игроков и рекомендации состава не реализованы.
