...
Статус: live validation пройдена; probe используется unified hourly updater.
...
**Token storage status: validated. API probe status: live validated.**

DPAPI token storage validation passed on 2026-07-19. Save-script был запущен обычной Windows user identity с `-Overwrite`; token введён через скрытый prompt. Script сообщил `DPAPI secret saved` и завершился с exit code `0`. Token и ciphertext не выводились, а настоящий secret остаётся вне workspace. Live probe и unified updater используют этот runner без публикации secret material.

Первый live runner attempt 2026-07-19 остановился до Python process и network request. Причиной был Windows PowerShell 5.1 module autoload. Runner теперь импортирует `Microsoft.PowerShell.Security` только из текущего `$PSHOME`; последующая live validation прошла успешно без повторного save и нового API key.
...
Execute создаёт только локальный каталог под `D:\coc\runs\api_probe`. Repository и `site` отклоняются. Unified updater читает completed run и отдельно строит allowlist-only public data.
...
Эта модель обеспечивает application-level all-or-nothing publication при обычных обработанных ошибках. Она не обещает полную crash-consistency при отключении питания или аварии операционной системы на любом этапе. Execute-mode прошёл live validation.
