...
Официально подтверждены Bearer JWT header, перечисленные endpoint paths, имена моделей, точные wire-field names и показанные Swagger types. Общий API base URL, version prefix, requiredness, nullability, точные enum values, status codes, error mapping, clan-tag alphabet/length и конкретные rate limits остаются неподтверждёнными.

Используемые статусы: `endpoint_verified`, `field_name_verified`, `field_type_verified`, `enum_values_unverified`, `requiredness_unverified`, `nullability_unverified`, `base_url_unverified`.
...
Подготовленный CLI и safety contract описаны в [clan_roster_probe.md](clan_roster_probe.md). Официальный clan profile path подтверждён, но `--endpoint-template` остаётся обязательным и execute принимает только `/clans/{clan_tag}`. Base URL остаётся обязательным ручным input без default со статусом `base_url_unverified`.

Authenticated Swagger review завершён, но настоящий API probe не выполнялся. До отдельно разрешённого минимального probe следует:

1. отдельно подтвердить base URL, version prefix, required/nullable, enum values, status codes и error mapping;
