...
- Root cause for the omitted safe source file was confirmed at the repository ignore layer: the filename matched `**/*token*`. An exact negation was added for the tracked source script only. Runtime revalidation remains pending.
- Настоящий token и DPAPI ciphertext остаются вне workspace в `%LOCALAPPDATA%\ClashClanAnalytics\secrets\coc_api_token.dpapi`; повторный refresh ещё не выполнялся.
