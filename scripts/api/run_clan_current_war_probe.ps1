...
$probePath = 'D:\coc\repo\scripts\api\probe_clan_current_war.py'
if (-not [System.IO.File]::Exists($probePath)) {
    throw 'Clan current-war probe entrypoint is missing.'
}
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $runTimestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $OutputDir = "D:\coc\runs\api_probe\clan_current_war\$runTimestamp"
}
...
$probeArguments = @(
    ...
    '--endpoint-template', '/clans/{clan_tag}/currentwar'
)
