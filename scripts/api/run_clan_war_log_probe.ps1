...
$probePath = 'D:\coc\repo\scripts\api\probe_clan_war_log.py'
if (-not [System.IO.File]::Exists($probePath)) {
    throw 'Clan war-log probe entrypoint is missing.'
}
if ([string]::IsNullOrWhiteSpace($OutputDir)) {
    $runTimestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $OutputDir = "D:\coc\runs\api_probe\clan_war_log\$runTimestamp"
}
...
$probeArguments = @(
    ...
    '--endpoint-template', '/clans/{clan_tag}/warlog'
)
