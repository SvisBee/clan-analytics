...
(() => {
  ...
  const weekPresentation = (week, { legacySchema = false } = {}) => {
    ...
    let title = current
      ? "Текущие показатели в игре"
      : "Последний снимок предыдущей календарной недели";
    ...
    let explanation = current
      ...
      : "Последние доступные значения счётчиков за предыдущую неделю по московскому времени. Игровой момент сброса пока не подтверждён.";
    const warnings = [];
    if (legacySchema) {
      ...
      if (week.coverage.stale_end_snapshot) {
        ...
        warnings.push("Последний доступный снимок был сделан раньше конца календарной недели.");
      }
